...
public class SinglePoolSimpleTests extends WarpSingleCPUTestBase {
	...
	public void testPoolAlloc_Reject() {
		...
		WarpPool p1 = warp.addPool("Pool1", 1000, 0, 0);
		...
		checkStepPool(steps.get(0), 0, 100, false, false);
		checkStepPool(steps.get(1), 0, 100,  true, false);
		checkStepPool(steps.get(2), 0,  50,  true, false);
	...
	public void testPoolAlloc_Execute() {
		...
		WarpPool p1 = warp.addPool("Pool1", 1000, 1, 0);
		...
		checkPool(p1, 2050, 2, 0);

		checkStepPool(steps.get(0), 0,  100, false, false);
		checkStepPool(steps.get(1), 0, 2100,  true, false);
		checkStepPool(steps.get(2), 0, 2050,  true, false);
	}

	@Test
	public void testPoolAlloc_Stop() {
		// add alloc/free pool with STOP_WORKING on overflow
		WarpPool p1 = warp.addPool("Pool1", 1000, 2, 0);
		...
		checkPool(p1, 100, 1, 0);

		checkStepPool(steps.get(0), 0,  100, false, false);
		checkStepPool(steps.get(1), 0,  100,  true, false);
		checkStepPool(steps.get(2), 0,  100,  true, false);
	}
